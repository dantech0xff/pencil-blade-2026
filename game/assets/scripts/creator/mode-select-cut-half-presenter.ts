...
import { getModeSelectCardResources } from '../domain/mode-select-resource-contract';
...
export type ModeSelectCutHalfPart = 'bottom' | 'top';
export type ModeSelectCutHalfDisposalReason =
  | 'fade-complete'
  | 'presenter-dispose'
  | Readonly<{ readonly boundary: DisposalBoundary; readonly type: 'bounds' }>;

export interface ModeSelectCutHalfResources {
  readonly bottom: LoadedGameRasterResource;
  readonly top: LoadedGameRasterResource;
}

export interface ModeSelectCutHalfPresenterInput {
  readonly assetTree: ClassicAssetTree;
  readonly modeIndex: ModeSelectIndex;
  readonly motion: ClassicCutHalfMotionPair;
  readonly resources: ModeSelectCutHalfResources;
}

export interface ModeSelectCutHalfPresenterLifecycle {
  readonly callAfterStep: (mutation: () => void) => void;
  readonly onDisposed?: (
    part: ModeSelectCutHalfPart,
    reason: ModeSelectCutHalfDisposalReason,
  ) => void;
}

export interface PresentedModeSelectCutHalf {
  readonly body: RigidBody2D;
  readonly collider: BoxCollider2D;
  readonly node: Node;
  readonly opacity: UIOpacity;
  readonly part: ModeSelectCutHalfPart;
  readonly sprite: Sprite;
}

type ModeSelectCutHalfTuple = readonly [PresentedCutHalf, PresentedCutHalf];

const MAX_OPACITY = 255;
const RADIANS_TO_DEGREES = 180 / Math.PI;
const RECOVERED_FRICTION = Math.fround(0.2);

/** Actual Creator CutFruit bodies for one accepted Mode Select card cut. */
export class ModeSelectCutHalfPresenter {
  readonly halves: ModeSelectCutHalfTuple;

  private attachedValue = false;
  private disposedValue = false;
  private elapsedActionSeconds = 0;
  private readonly lifecycle: ModeSelectCutHalfPresenterLifecycle;

  private constructor(
    input: ModeSelectCutHalfPresenterInput,
    lifecycle: ModeSelectCutHalfPresenterLifecycle,
  ) {
  ...
  static create(
    ...
    return new ModeSelectCutHalfPresenter(input, lifecycle);
  }
  ...
  attach(parent: Node, firstSiblingIndex: number): void {
    if (!isValid(parent, true) || !parent.activeInHierarchy) {
      throw new Error('Mode Select cut-half parent must be valid and active');
    }
    ...
    if (
      ...
      throw new Error('Mode Select cut-half presenter cannot attach from its current state');
    }
  ...
  evaluateBounds(viewport: Readonly<{
  ...
  dispose(): boolean {
    ...
    for (const half of this.halves) {
      ...
      this.lifecycle.onDisposed?.(half.part, 'presenter-dispose');
    }
    return true;
  }

  private queueAll(reason: ModeSelectCutHalfDisposalReason): void {
    for (const half of this.halves) {
...
class PresentedCutHalf implements PresentedModeSelectCutHalf {
  readonly body: RigidBody2D;
  readonly collider: BoxCollider2D;
  readonly node: Node;
  readonly opacity: UIOpacity;
  ...
  constructor(
    ...
    this.node = new Node(`ModeSelectCutHalf-${part}`);
  ...
  applyInitialCentreImpulse(): void {
    if (this.impulseApplied) {
      throw new Error(`Mode Select ${this.part} cut-half impulse was already applied`);
    }
...
function assertInput(input: ModeSelectCutHalfPresenterInput): void {
  assertObject(input, 'input');
  const expected = getModeSelectCardResources(input.modeIndex, input.assetTree);
...
function assertLoadedRaster(
  ...
  if (
    ...
    throw new Error(`${label} must match the exact loaded Mode Select cut-half resource`);
  }
}
...
function assertMotion(motion: ClassicCutHalfMotionPair): void {
  ...
  for (const [label, value] of [
    ...
    assertFinite(value.angularVelocityRadiansPerSecond, `${label}.angularVelocity`);
    assertFinite(value.positionWorldUnits.x, `${label}.position.x`);
    assertFinite(value.positionWorldUnits.y, `${label}.position.y`);
    assertFinite(value.impulseNewtonSeconds.x, `${label}.impulse.x`);
    assertFinite(value.impulseNewtonSeconds.y, `${label}.impulse.y`);
  }
}

function assertLifecycle(lifecycle: ModeSelectCutHalfPresenterLifecycle): void {
  assertObject(lifecycle, 'lifecycle');
  if (typeof lifecycle.callAfterStep !== 'function') {
    throw new TypeError('Mode Select cut-half lifecycle requires callAfterStep()');
  }
  if (lifecycle.onDisposed !== undefined && typeof lifecycle.onDisposed !== 'function') {
    throw new TypeError('Mode Select cut-half onDisposed must be a function when provided');
  }
}
