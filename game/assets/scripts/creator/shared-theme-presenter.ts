...
export class SharedThemePresenter {
  readonly root: Node;
  ...
  private constructor(
    ...
    this.root = new Node('SharedThemeRoot');
    this.spriteNode = new Node('SharedThemeSprite');
  ...
  static create(
    resources: readonly LoadedGameRasterResource[],
    selectedIndex = SHARED_THEME_DEFAULT_INDEX,
  ): SharedThemePresenter {
    assertResourceFamily(resources);
    assertIndex(selectedIndex, 0, 9, 'selected theme index');
    return new SharedThemePresenter(resources, selectedIndex);
  }
  ...
  attach(parent: Node, siblingIndex: number): void {
    ...
    if (!isValid(parent, true) || !parent.active) {
      throw new Error('Shared theme parent must be valid and active');
    }
    if (this.root.parent !== null) {
      throw new Error('Shared theme is already attached');
    }
  ...
  select(index: number): void {
    ...
    const resource = requireResource(this.resources[index], index);
  ...
  private assertUsable(action: string): void {
    if (this.disposedValue || !isValid(this.root, true)) {
      throw new Error(`Disposed shared theme cannot ${action}`);
    }
  }
}

function assertResourceFamily(resources: readonly LoadedGameRasterResource[]): void {
  if (!Array.isArray(resources) || resources.length !== 10) {
    throw new RangeError('Shared theme resources must contain 10 rasters');
  }
  const tree = resourceTree(resources[0]);
  for (let index = 0; index < resources.length; index += 1) {
    const expected = getSharedThemeResource(tree, index);
    const resource = requireResource(resources[index], index);
    if (
      ...
      throw new RangeError(`Shared theme resource ${index} is not the recovered raster`);
    }
  }
}
...
function resourceTree(resource: LoadedGameRasterResource | undefined): '480x800' | '720x1280' {
  ...
  throw new RangeError('Shared theme resource must belong to one canonical asset tree');
}
...
function requireResource(
  ...
  if (resource === undefined || !isValid(resource.spriteFrame, true)) {
    throw new Error(`Missing loaded shared theme SpriteFrame ${index}`);
  }
  return resource;
}
