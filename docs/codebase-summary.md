...
Generated from `repomix-output.xml` on 2026-07-23.
...
| `game/` | Creator 3.8.8 project with pure TypeScript domain modules, Creator adapters, exact recovered APK assets, and production routes for Classic, Crazy, and Classic Bird. |
| `tests/reconstruction/vertical-slice/` | Contract tests for the recovered menu/shared-scene, Classic, Crazy, and Classic Bird slices plus source-boundary audits. |
...
| `tests/reconstruction/vertical-slice/*.test.ts` | `876/876` deterministic regressions for the current vertical slice, including fault-injected Classic, Crazy, and Classic Bird lifecycle transactions. |
...
- Building Classic, Crazy, and Classic Bird as clean TypeScript with Creator adapters at the boundary.
- Rendering the exact Classic score HUD with authoritative best-score updates seeded from persisted `classic_best_1`.
- Running the shared BaseBird/BirdBlade substrate for Classic Bird mode `3`; modes `2`, `4`, and `5` remain fail closed.
...
- The current deterministic suite is `876/876`; the static inventory/source/staging/archive workflow is `14/14`; reconstruction policy positive plus `4/4` negative fixtures and strict Creator TypeScript hold at the checkpoint; `git diff --check` is clean. The Result-entry rollback reacquisition P1 is resolved.
- Fresh Creator Preview now reaches Main Menu → Mode Select → Classic Bird, live Bird/blade/particles, natural Game Over → Result, Result Retry, Pause Resume, and Replay/Quit with zero errors.
...
- Shared BaseBird/BirdBlade plus Crazy Bird next, then Combo Bird, and GN Style.
