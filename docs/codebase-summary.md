...
status is maintained manually and is updated through the 2026-07-24 cosmetic-settings checkpoint.
...
| `game/assets/scripts/creator/recovered-app-shell-controller.ts` | Persistent app shell and transactional Main Menu / Mode Select / gameplay screen replacement. |
...
| `tests/reconstruction/vertical-slice/*.test.ts` | `1157/1157` deterministic regressions for the GN Style checkpoint and executable lifecycle faults across all routes. |
...
- The GN Style checkpoint is `1157/1157` full vertical-slice tests, `43/43` resource/build/catalog tests, the unchanged `14/14` inventory/evidence workflow in `217s`, reconstruction policy positive plus `4/4` negative fixtures, native static analysis `7/7`, strict Creator TypeScript, and clean diff hygiene.
- Fresh Creator Preview reaches Main Menu → Mode Select → GN Style → exact intro → live cuts, score, music, and particles → Pause/Resume/Replay → Pause Quit → Main Menu → repeated entry → natural Time Up → Result Retry → Result Menu. DevTools reports zero application/runtime errors; one unrelated Chrome extension error remains outside the game.
...
- Full Settings coverage beyond the implemented subset and separate mode-unlock keys, including the first-launch `flag` bootstrap and the remaining options UI.
- Blade IDs `1`-`17` presentation and remaining cosmetic resource consumers.
