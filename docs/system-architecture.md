...
mode `4`, or the independent production Combo Bird loop through mode `5`. Five of six routes
now replace screens transactionally and support their recovered Retry/Replay/Quit/Main Menu paths.
...
Automated verification reaches `70/70` focused Combo Bird tests, `1030/1030` full
...
hygiene are clean. The final Creator-served Preview reaches Main Menu → Mode Select → Combo
Bird → live Bird/type-3 gameplay → Pause/Resume/Replay → Pause Quit → Main Menu; final Cocos
console counters are `0/0/0/0`. The full gameplay and presentation layer remains incomplete
because GN Style mode `2`, full consumer coverage, and a real Android build are still open.
...
Classic Bird, Crazy Bird, or Combo Bird activation only after Mode Select hands off recovered
mode `0`, `1`, `3`, `4`, or `5`; component enable/disable is not the pause boundary.
...
| Creator resource bundle | `game/assets/game/` | Exact staged bytes for all 862 recovered APK game assets; reviewed Classic, menu/shared-scene, Crazy, Combo, and Bird type-1/type-2/type-3 subsets have production consumers while full consumer coverage remains open. |
| Initial scene bridge | `game/assets/scenes/classic.scene` | Editor-serialized Canvas with blade input, passive Classic, shared Crazy modes `1`/`4`, Classic Bird and Combo Bird runtime components, and the persistent recovered app shell. |
...
| Build boundary | Source-boundary tests reject trackable legacy integration. The separate fail-closed archive audit hashes every entry, parses ZIP records exactly, recurses through bounded nested archives, and inspects ELF payloads; the unchanged inventory/source/staging/archive workflow is `14/14`, `tests/*.mjs` are `38/38`, the full vertical slice is `1030/1030`, and strict Creator TypeScript is green. |
...
- The persistent shell now preserves the recovered equal-z append order Background → Leaf →
  Theme → current screen, and all live routes stay transactional. The current Combo Bird
  checkpoint is confirmed in a fresh Creator-served Browser Preview.
- Mode `2` retains its recovered GN Style card and lock but has no destination presenter. It
  deliberately fails closed rather than aliasing another mode or inventing gameplay. GN Style
  is the next and only remaining production route.
...
- Exact recovered APK resources are imported and the Classic, Crazy, Classic Bird, Crazy Bird,
  and Combo Bird consumers are integrated; most of the 862 assets still need consumer mapping. The
