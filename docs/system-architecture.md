...
mode `4`. All four routes replace screens transactionally and support their recovered
Retry/Replay/Quit/Main Menu paths. Crazy Bird profiles the shared Crazy controllers with
BirdBlade type `2`, the exact 17-raster Bird resource set, distinct mode-4 objectives,
`bird_crazy_best_1..3`, and the float32 `0.8` result reward.

Automated verification reaches `952/952` vertical-slice tests, `38/38` resource/build/catalog
tests, and `14/14` inventory/evidence cases in `217s`. Reconstruction policy positive plus
`4/4` negative fixtures, native static analysis `7/7`, strict Creator TypeScript, diff hygiene,
and independent runtime review are clean. The final Creator-served Preview reaches Main Menu →
Mode Select → Crazy Bird → live Bird/type-2 gameplay → Pause/Resume/Replay → Pause Quit →
Main Menu; the Cocos console is empty and the post-gesture DevTools check reports `0` messages.
The full gameplay and presentation layer remains incomplete because modes `2` and `5`, full
consumer coverage, and a real Android build are still open.
...
Classic Bird, or Crazy Bird activation only after Mode Select hands off recovered mode `0`,
`1`, `3`, or `4`; component enable/disable is not the pause boundary.
...
| Creator resource bundle | `game/assets/game/` | Exact staged bytes for all 862 recovered APK game assets; reviewed Classic, menu/shared-scene, Crazy, and Bird type-1/type-2 subsets have production consumers while full consumer coverage remains open. |
| Initial scene bridge | `game/assets/scenes/classic.scene` | Editor-serialized Canvas with blade input, passive Classic, shared Crazy modes `1`/`4`, Classic Bird runtime components, and the persistent recovered app shell. |
...
| Build boundary | Source-boundary tests reject trackable legacy integration. The separate fail-closed archive audit hashes every entry, parses ZIP records exactly, recurses through bounded nested archives, and inspects ELF payloads; the complete inventory/source/staging/archive workflow is `14/14`, `tests/*.mjs` are `38/38`, and strict Creator TypeScript plus runtime review are both green. |
...
- The persistent shell now preserves the recovered equal-z append order Background → Leaf →
  Theme → current screen, and all live routes stay transactional. The current Crazy Bird
  checkpoint is confirmed in a fresh Creator-served Browser Preview.
- Modes `2` and `5` retain recovered Mode Select cards and locks but have no destination
  presenters. They deliberately fail closed rather than aliasing Classic or inventing gameplay.
  Phase 6 continues with Combo Bird, then GN Style.
...
- Exact recovered APK resources are imported and the first Classic, Crazy, Classic Bird, and
  Crazy Bird consumers are integrated; most of the 862 assets still need consumer mapping. The
...
- Exact result-entry layout/ranking/actions, reward visuals, app-hide persistence, Result-to-Main
  replacement, same-parent Retry reconstruction, and Bird modes `3`/`4` are integrated behind
