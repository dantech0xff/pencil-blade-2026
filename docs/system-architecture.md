...
- The Classic slice persists only `total_coins` and `classic_best_1/2/3`. The remaining native
  Settings fields, first-launch initialization checkpoint, and Main Menu exit-save are open.
- Exact result-entry layout/ranking/actions, reward visuals, and the app-hide persistence
  checkpoint are integrated. MainMenu replacement remains open, and Retry currently reloads
  `classic.scene` instead of reconstructing Classic under the same native parent.
